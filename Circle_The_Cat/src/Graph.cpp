        ...
        for (auto neighbor = m_adj[VertexInProcess].begin(); neighbor != m_adj[VertexInProcess].end(); ++neighbor)
        {
            if (!visited[*neighbor])
            {
                ...
                if (isEndOfBoard(*neighbor)) {
                    int goTo = getTheNodeToGo(startNode, parentsOfNode, *neighbor);
                    return goTo;
                }
            }
        }
...
//-----------getTheCellToGo-----------------
int Graph::getTheNodeToGo(const int start, const vector<pair<int, int>> parentsOfNode,const int adjecent) {

    int sonOfRoot, 
        i = parentsOfNode.size() - 1;
